...
namespace DentistProject.WebAPI.Controllers
{
    ...
    public class BlogController : ControllerBase
    {
        ...
        public BlogController(IBlogService blogService, IAccountService accountService,IHttpContextAccessor httpContext)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> GetAll([FromQuery] int? page, [FromBody] BlogFilter? filter)
        {
            if (!methods.Contains(EMethod.BlogList))
            {
                return Unauthorized();
            }
        ...
        [HttpPost("Count")]
        public async Task<IActionResult> Count([FromBody] BlogFilter? filter)
        {
            if (!methods.Contains(EMethod.BlogCount))
            {
                return Unauthorized();
            }
            var result = await _blogService.Count(filter);
            if (result.Status == Dtos.Enum.EResultStatus.Success)
            {
                return Ok(result.Result);
            }
        ...
        {
            if (!methods.Contains(EMethod.BlogGet))
            {
                return Unauthorized();
            }
        ...
        //{
        //    if (!methods.Contains(EMethod.BlogGetValidItem))
        //    {
        //        return Unauthorized();
        //    }
        //    var result = await _blogService.GetAll(new Dtos.Filter.LoadMoreFilter<BlogFilter>
        //    {
        //        ContentCount = 1,
        //        PageCount = 0,
        ...
        //{
        //    if (!methods.Contains(EMethod.BlogChangePhoto))
        //    {
        //        return Unauthorized();
        //    }
        ...
        {
            if (!methods.Contains(EMethod.BlogAdd))
            {
                return Unauthorized();
            }
        ...
        {
            if (!methods.Contains(EMethod.BlogUpdate))
            {
                return Unauthorized();
            }
        ...
        {
            if (!methods.Contains(EMethod.BlogDelete))
            {
                return Unauthorized();
            }
